# 业务视角，统计business_id组的频次，以频次为特征使用层次聚类，删除‘结束’字段
import csv
import sqlite3
import pandas as pd
...
# parameter
W1 = {'检索提取': 1, '重打牌': 14, '行李交运': 14, '返回': 1,
      '旅客拉下': 14, '下一步值机': 1, '值机接收': 14, '客票详情': 7,
      '管理旅客备注': 14, '旅客详情': 7, '值机员综合办理统计': 7, '航班详情': 7,
      '座位锁放': 14, '旅客升降舱': 14, '办理历史': 7, '座位处理': 14, '航班值机情况查询': 1}
...
with open(FILE, mode='w', newline='', encoding='gbk') as file:
    ...
    for IndexSets in tqdm(result_index):
        ...
        route_dict = {key: 1 for key in route_counter}
...
deleted = ['结束', '返回', '客票详情', '旅客详情', '航班详情']
CheckInData_w = CheckInData_f.loc[:, [col for col in choose_cols if col not in deleted]].fillna(0)
# multiple weights: W
for col in [col for col in choose_cols if col not in deleted]:
    if col in W1.keys():
        # CheckInData_w[col] = CheckInData_w[col] * round(W1[col] / sum(W1.values()) * 100, 0)
        CheckInData_w[col] = CheckInData_w[col] * W1[col]
    else:
        pass
...
CheckInData_w.to_excel('xls/agg_w1_120DT_id_group_cluster01_%s.xlsx' % N_CLUSTER, index=False)
